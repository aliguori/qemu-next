...
static void collect_property(Object *obj, const char *name,
                             ...
                             void *opaque)
{
    ...
    info->typename = g_strdup(typename);
...
int qdev_device_help(QemuOpts *opts)
{
    ...
    object_property_foreach(obj, collect_property, &props);

    for (i = props; i; i = i->next) {
        PropertyData *prop = i->data;
        const char *name = prop->name;
        char *typename = g_strdup(prop->typename);

        if (strstart(prop->name, "legacy-", NULL) && i->next) {
            PropertyData *next = i->next->data;

            if (strcmp(next->name, prop->name + 7) == 0) {
                name = next->name;
                g_free(typename);
                typename = g_strdup(prop->typename + 7);
