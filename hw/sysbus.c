...
void sysbus_mmio_map(SysBusDevice *dev, int n, hw_addr_t addr)
{
    ...
    if (dev->mmio[n].addr != (hw_addr_t)-1) {
...
void sysbus_init_mmio(SysBusDevice *dev, hw_addr_t size, int iofunc)
{
    int n;
...
void sysbus_init_mmio_cb(SysBusDevice *dev, hw_addr_t size,
                         mmio_mapfunc cb)
{
    int n;
